...
namespace Myre.Collections
{
    ...
    {
        #region fields
        private readonly Dictionary<NameWithType, IBox> _values = new();
        ...
        public Box<T> GetOrCreate<T>(TypedName<T> key, T? defaultValue = default)
        {
            var box = Get(key.Name, typeof(T));

            if (box is Box<T> box1)
                return box1;
        ...
        {
            if (_values.TryGetValue(new NameWithType(key.Name, typeof(T)), out var box))
            {
                value = (Box<T>)box;
                return true;
            }
        ...
        {
            if (_values.TryGetValue(new NameWithType(key, type), out var box))
                return box;
            return null;
        }
        #endregion
