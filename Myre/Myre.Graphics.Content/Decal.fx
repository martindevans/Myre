...
float3 ClipPixelNormal(float3 worldPosition, out float3 tangent, out float3 binormal)
{
	...
	//Do not draw decal if projection direction of decal is too different to surface normal
	float d = dot(normal, DecalDirection);
	clip(d - DecalDirectionClip);
...
float4 DiffuseValue(float2 decalTexCoord)
{
	return tex2D(diffuseSampler, decalTexCoord);
}
	...
	out float4 out_diffuse : COLOR1)
{
	...
	float3 pixelNormal = ClipPixelNormal(worldPosition, pixelTangent, pixelBinormal);

	if (outputDiffuse)
		out_diffuse = DiffuseValue(decalTexCoord);
	else
