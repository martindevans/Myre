...
float3 GetColor(float3 worldPosition)
{
	float4 transformed = mul(worldPosition, PlaneMatrix);
	return tex2D(textureSample, transformed.xy).rgb;
}

float4 CalculateLighting(float2 texCoord, float3 viewPosition, float3 worldPosition)
{
	...
	float3 light = GetColor(worldPosition) * attenuation;
				...
				out float4 out_Colour : COLOR0)
{
	...
	float4 projectedPos = float4(texCoord.x, texCoord.y, sampledDepth.x, 1);
	float3 vpos = projectedPos.xyz / projectedPos.w;
	float4 position = mul(viewPosition, inverseView);
	out_Colour = float4(position.xyz / position.w, 1);


	//out_Colour = CalculateLighting(texCoord, viewPosition, in_PositionW);
}
			...
			out float4 out_Colour : COLOR0)
{
	...
	out_Colour = float4(0, 1, 0, 1);//CalculateLighting(in_TexCoord, viewPosition, in_PositionW);
}
